...
FROM (SELECT DISTINCT CustomerName from [Sales].[Customers]) as t

SET @command = N'SELECT * FROM (
	SELECT DISTINCT 
		convert(varchar,DATEADD(dd,-(day(i.InvoiceDate)-1),i.InvoiceDate), 104) as InvoiceMonth, 
		SUM(l.Quantity) as tot,
		CustomerName
...
PIVOT (sum(tot) for CustomerName in(' + @column + '
)) as pvt
